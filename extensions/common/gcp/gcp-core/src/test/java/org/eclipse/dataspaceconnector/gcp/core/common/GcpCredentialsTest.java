...
class GcpCredentialsTest {
    ...
    void testResolveGoogleCredentialWhenTokenKeyNameIsProvided() {
        var dataAddress = DataAddress.Builder.newInstance().type(GcsStoreSchema.TYPE)
                .keyName(accessTokenKeyName).build();
        var gcpCred = gcpCredential.resolveGoogleCredentialsFromDataAddress(dataAddress);
        assert (gcpCred != null);
    }
    ...
    void testResolveGoogleCredentialWhenInvalidTokenIsProvided() {
        var dataAddress = DataAddress.Builder.newInstance().type(GcsStoreSchema.TYPE)
                .keyName(invalidAccessTokenKeyName).build();
        Exception thrown = assertThrows(EdcException.class, () -> gcpCredential.resolveGoogleCredentialsFromDataAddress(dataAddress));
    ...
    void testResolveGoogleCredentialPriorityWhenTokenIsInvalid() {
        var dataAddress = DataAddress.Builder.newInstance().type(GcsStoreSchema.TYPE)
                .keyName(invalidAccessTokenKeyName)
                .property(GcsStoreSchema.SERVICE_ACCOUNT_KEY_NAME, serviceAccountKeyName)
                .build();
        Exception thrown = assertThrows(EdcException.class, () -> gcpCredential.resolveGoogleCredentialsFromDataAddress(dataAddress));
        assert (thrown.getMessage().contains("valid GcpAccessToken format"));
    }
    ...
    void testResolveGoogleCredentialWhenServiceAccountKeyNameIsProvided() {
        var dataAddress = DataAddress.Builder.newInstance().type(GcsStoreSchema.TYPE)
                .property(GcsStoreSchema.SERVICE_ACCOUNT_KEY_NAME, serviceAccountKeyName).build();
        var gcpCred = gcpCredential.resolveGoogleCredentialsFromDataAddress(dataAddress);
        assert (gcpCred != null);
    }
    ...
    void testResolveGoogleCredentialWhenInvalidServiceAccountKeyNameIsProvided() {
        var dataAddress = DataAddress.Builder.newInstance().type(GcsStoreSchema.TYPE)
                .property(GcsStoreSchema.SERVICE_ACCOUNT_KEY_NAME, invalidServiceAccountKeyName).build();
        Exception thrown = assertThrows(GcpException.class, () -> gcpCredential.resolveGoogleCredentialsFromDataAddress(dataAddress));
    }

    @Test
    void testResolveGoogleCredentialWhenServiceAccountValueIsProvided() {
        var dataAddress = DataAddress.Builder.newInstance().type(GcsStoreSchema.TYPE)
                .property(GcsStoreSchema.SERVICE_ACCOUNT_VALUE, serviceAccountFileInB64).build();
        var gcpCred = gcpCredential.resolveGoogleCredentialsFromDataAddress(dataAddress);
        assert (gcpCred != null);
    }
    ...
    void testResolveGoogleCredentialWhenInvalidServiceAccountValueIsProvided() {
        var dataAddress = DataAddress.Builder.newInstance().type(GcsStoreSchema.TYPE)
                .property(GcsStoreSchema.SERVICE_ACCOUNT_VALUE, serviceAccountFileInB64 + "makeItWrongB64").build();
        Exception thrown = assertThrows(EdcException.class, () -> gcpCredential.resolveGoogleCredentialsFromDataAddress(dataAddress));
